...
export const handler = async function (
  ...
  for (let ind = 0; ind < connectionItems.length; ind++) {
    console.log(`connectionId => ${connectionItems[ind].connectionId}`);
    try {
      const postToConnectionCommandResp = await apiGwManApiClient.send(
        new PostToConnectionCommand({
          ConnectionId: connectionItems[ind].connectionId,
          Data: textEncoder.encode(
            "A new review published for book with id 123"
          ),
        })
      );
      console.log(
        `postToConnectionCommand resp => ${JSON.stringify(
          postToConnectionCommandResp
        )}`
      );
    } catch (err) {
      console.error(`Error while posting to connection => ${err}`);
    }
  }
  return {
    statusCode: 200,
